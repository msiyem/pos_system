...
export default function CustomerCard({
  ...
  return (
    <div className={`min-h-[185px]  ring-0 border p-3 rounded-xl hover:shadow-lg space-y-2  gap-2 w-full overflow-hidden
    ...
    `}>
      <div className="flex justify-between w-full">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <div
            title={name}
            className="text-lg font-semibold mb-1 font-mono truncate  "
          >
            {name}
      ...
      <div className="flex justify-between mt-auto text-[12px]  w-full">
            <div className="border border-gray-300 rounded-md px-1 w-fit">
              Last Visit: {lastVisit || null}
            </div>
            <div>
              {dues > 0 ? (
                <div className="ring-0  rounded-lg  border shadow px-1 text-red-500 cursor-pointer w-fit">
                  <span>Dues:</span> <span>{dues}৳</span>
                </div>
              ) : (
                <div className="ring-0 rounded-lg  border shadow px-1 text-blue-600 cursor-pointer w-fit">
                  No Dues
                </div>
              )}
            </div>
